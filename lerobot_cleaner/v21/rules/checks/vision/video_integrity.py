...
from lerobot_cleaner.v21.video_utils import VideoError, count_frames, is_decodable


class VideoIntegrityRule(CheckRule):
    ...
    def check(self, work: EpisodeWork, context=None) -> CheckResult:
        ...
        for vkey, vpath in work.ref.video_paths.items():
            if not vpath.exists():
                ...
                return self.result(work, False, {"frame_counts": counts, "failed_view": vkey})
        ...
        return self.result(work, not work.dropped, {"frame_counts": counts})
