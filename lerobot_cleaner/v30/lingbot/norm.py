...
def validate_norm_output(path: Path, dataset: Path, robot_config: Path, train_config: Path):
    ...
    robot = LingBotRobotConfigSchema.from_yaml(robot_config, info["features"])
    train = yaml.safe_load(train_config.read_text(encoding="utf-8"))
    delta_keys = {feature.target_name for feature in robot.actions if feature.subtract_state}
